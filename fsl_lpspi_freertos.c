...
static void LPSPI_RTOS_Callback(LPSPI_Type *base, lpspi_master_handle_t *drv_handle, status_t status, void *userData) {
  ...
  handle = (lpspi_rtos_handle_t*) userData;

  handle->async_status = status;

  handle->store.transmitDone = (status == kStatus_Success) ? true : false;
  if (handle->store.SpiCallback != NULL) {
    handle->store.SpiCallback(handle->store.SpiCBParam, 0, NULL);
  }
...
status_t LPSPI_RTOS_Init(lpspi_rtos_handle_t *handle, LPSPI_Type *base, const lpspi_master_config_t *masterConfig, uint32_t srcClock_Hz) {
  ...
  (void) memset(handle, 0, (sizeof(lpspi_rtos_handle_t) - sizeof(fsl_lpspi_freertos_store_t)));
